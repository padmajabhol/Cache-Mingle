...
def test_create_value():
    ...
    key = " "
    assert client.put(f"/cache/{key}", json=data2).status_code == 404
...
def test_get_value():
    ...
    assert response.json() == data
    assert client.get("/cache/2").status_code == 404
...
def test_set_bulk_value():
    ...
    assert client.put(f"/bulkcache", json=request_body2).status_code == 404
